...
export const updateStatistics = (statistics: StatisticsObjType) => {
    let statisticsHtml = `
        ...
        <span class="statistics__item">Living: ${statistics.livingCells} (${Math.floor((statistics.livingCells / statistics.totalCells) * 100)}%)</span>
        <span class="statistics__item">Generation: ${statistics.generation}</span>
        <span class="statistics__item">Paint speed: ${getAvgTime(paintTimes, paintSpeedsToRemember, statistics.paintTime)} ms</span>
        <span class="statistics__item">Calculation speed: ${getAvgTime(calcTimes, calcSpeedsToRemember, statistics.calcTime)} ms</span>
    `;
